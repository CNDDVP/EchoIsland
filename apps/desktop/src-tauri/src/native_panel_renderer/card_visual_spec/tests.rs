...
fn card_spec_exposes_shared_action_hint_pill_palette_and_metrics() {
    let spec = card_visual_action_hint_paint_spec("在终端中允许 / 拒绝").expect("action hint spec");
...
fn card_spec_exposes_shared_action_hint_layout() {
    ...
    let layout = card_visual_action_hint_layout(frame, "检查").expect("action hint layout");
    ...
    assert_eq!(layout.text_max_width, 20.0);
...
#[test]
fn card_spec_preserves_long_agent_and_chinese_status_badges() {
    for text in ["Antigravity", "需要批准", "等待你的操作"] {
        let badge = CardVisualBadgeSpec {
            role: CardVisualBadgeRole::Source,
            text: text.to_string(),
            emphasized: false,
        ...
        let estimated_text_width = text
            .chars()
            .map(|c| if c.is_ascii() { 6.2 } else { 11.0 })
            .sum::<f64>();
        assert!(layout.badge_frame.width >= estimated_text_width + 18.0);
        assert!(layout.text_max_width >= estimated_text_width);
    }
}
