...
use crate::{
    ...
    native_panel_renderer::{
        ...
        visual_primitives::{
            ...
            NativePanelVisualPrimitive, NativePanelVisualTextAlignment, NativePanelVisualTextRole,
...
fn compact_visual_plan_uses_shared_active_count_marquee_frame() {
    ...
    input.compact_bar_frame.width = 253.0;
    input.compact_bar_frame.height = 37.0;
    input.active_count = "AB".to_string();
    input.active_count_elapsed_ms = ACTIVE_COUNT_SCROLL_HOLD_MS + (ACTIVE_COUNT_SCROLL_MOVE_MS / 2);
    let plan = resolve_native_panel_visual_plan(&input);
    let current = plan
        .primitives
        .iter()
        .find_map(|primitive| match primitive {
            NativePanelVisualPrimitive::Text { origin, text, .. } if text == "A" => Some(*origin),
            _ => None,
        })
    ...
    let next = plan
        ...
        .iter()
        .find_map(|primitive| match primitive {
            NativePanelVisualPrimitive::Text { origin, text, .. } if text == "B" => Some(*origin),
            _ => None,
        })
...
fn compact_visual_plan_positions_active_count_marquee_in_slot() {
    ...
    input.compact_bar_frame.width = 253.0;
    input.compact_bar_frame.height = 37.0;
    input.active_count = "AA".to_string();
...
#[test]
fn compact_visual_plan_keeps_every_numeric_count_on_one_line() {
    for count in ["10", "16", "22", "23", "64", "100"] {
        for elapsed_ms in [
            0,
            ACTIVE_COUNT_SCROLL_HOLD_MS + ACTIVE_COUNT_SCROLL_MOVE_MS / 2,
        ] {
            let mut input = visual_input(NativePanelVisualDisplayMode::Compact);
            input.active_count = count.to_string();
            input.total_count = "100".to_string();
            input.active_count_elapsed_ms = elapsed_ms;
            let plan = resolve_native_panel_visual_plan(&input);
            let counters = plan
                .primitives
                .iter()
                .filter_map(|primitive| match primitive {
                    NativePanelVisualPrimitive::Text {
                        ...
                        max_width,
                        ..
                    } => Some((text, origin, max_width)),
                    _ => None,
                })
                .collect::<Vec<_>>();
            assert_eq!(counters.len(), 1);
            assert_eq!(counters[0].0, count);
            assert_eq!(
                counters[0].1.y,
                input.compact_bar_frame.y + compact_digit_y(input.compact_bar_frame.height)
            );
            assert!(
                *counters[0].2
                    >= crate::native_panel_core::resolve_estimated_text_width(count, 15.0)
            );
            assert!(!plan.primitives.iter().any(|primitive| matches!(
                primitive,
                NativePanelVisualPrimitive::Text {
