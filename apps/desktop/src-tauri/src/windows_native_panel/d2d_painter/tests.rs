...
fn direct2d_painter_skeleton_routes_compact_text_to_directwrite_requests() {
    ...
    assert!(requests.iter().all(|request| {
        request.fonts.primary == echoisland_i18n::WINDOWS_UI_FONT
            && request.fonts.fallback == "Segoe UI Variable"
    }));
}
...
fn direct2d_painter_skeleton_routes_expanded_cards_to_directwrite_requests() {
    ...
    assert!(requests.iter().any(|request| request.text == "已完成"));
    assert!(requests.iter().any(|request| request.text == "任务完成"));
