...
#[test]
fn windows_renderer_refresh_preserves_selected_monitor_physical_context() {
    let mut renderer = super::WindowsNativePanelRenderer::default();
    ...
    let screen_physical_frame = PanelRect {
        x: 3840.0,
        y: 0.0,
        width: 1920.0,
        height: 1080.0,
    };
    renderer.last_host_window_descriptor = Some(NativePanelHostWindowDescriptor {
        visible: true,
        preferred_display_index: 1,
        screen_frame: Some(screen_frame),
        screen_scale_factor: Some(1.0),
        screen_physical_frame: Some(screen_physical_frame),
        shared_body_height: None,
        timeline: None,
    });
    renderer.update_screen_frame(Some(screen_frame));
    ...
    let state = renderer.last_window_state.expect("cached window state");
    assert_eq!(state.screen_scale_factor, Some(1.0));
    assert_eq!(state.screen_physical_frame, Some(screen_physical_frame));
    assert_eq!(state.preferred_display_index, 1);
...
fn windows_runtime_hover_expanded_panel_switches_to_new_completion_status_message() {
    ...
    completed_snapshot.sessions[0].last_assistant_message = Some("已完成".to_string());
    let mut runtime = super::WindowsNativePanelRuntime {
        panel_state: PanelState {
            expanded: true,
            pointer_inside_since: Some(
