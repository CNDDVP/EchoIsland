...
impl WindowsNativePanelWindowShell {
    ...
    pub(super) fn active_count_marquee_needs_refresh(&self) -> bool {
        self.lightweight_refresh_visible()
            && self.display_snapshot.as_ref().is_some_and(|display| {
                active_count_needs_marquee(&display.visual_input.active_count)
            })
    }
    ...
    pub(super) fn refresh_active_count_marquee(&mut self, elapsed_ms: u128) -> bool {
        ...
        if !active_count_needs_marquee(&display.visual_input.active_count) {
            return false;
        }
...
fn active_count_needs_marquee(text: &str) -> bool {
    text.chars().count() > 1 && !text.chars().all(|character| character.is_ascii_digit())
}
