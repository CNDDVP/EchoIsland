    ...
    tauri::async_runtime::spawn(async move {
        for delay_ms in [0, 100, 320, 900] {
            ...
            warn!(
                ...
                "native event-lifecycle snapshot refresh"
            );
            if let Err(error) = native_panel_backend.update_snapshot(&app_handle, &snapshot) {
                warn!(error = %error, "failed to refresh native island after lifecycle event");
            }
        }
    });
}
...
#[cfg(any(target_os = "macos", target_os = "windows"))]
pub async fn refresh_native_ui_after_event<R: tauri::Runtime + 'static>(
    app_handle: AppHandle<R>,
    runtime: Arc<SharedRuntime>,
    event_name: &str,
) {
    if !should_refresh_native_ui_after_event(event_name) {
        return;
    }
    let native_panel_backend = current_native_panel_runtime_backend();
    if !native_panel_backend.native_ui_enabled() {
    ...
    let snapshot = runtime.snapshot().await;
    if let Err(error) = native_panel_backend.update_snapshot(&app_handle, &snapshot) {
        warn!(error = %error, "failed to refresh native island after completion event");
    }
}
...
mod tests {
    ...
    #[test]
    fn refreshes_native_ui_synchronously_after_completion_events() {
        assert!(should_refresh_native_ui_after_event("Stop"));
        assert!(should_refresh_native_ui_after_event("SessionEnd"));
        assert!(!should_refresh_native_ui_after_event("UserPromptSubmit"));
    }
}
