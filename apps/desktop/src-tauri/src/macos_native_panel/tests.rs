...
use super::panel_constants::{
    CARD_FOCUS_CLICK_DEBOUNCE_MS, DEFAULT_COMPACT_PILL_HEIGHT, DEFAULT_COMPACT_PILL_WIDTH,
    DEFAULT_EXPANDED_PILL_WIDTH, DEFAULT_PANEL_CANVAS_WIDTH, EXPANDED_PILL_WIDTH_DELTA,
    HOVER_DELAY_MS, PANEL_CLOSE_MORPH_DELAY_MS, PANEL_CLOSE_SHOULDER_DELAY_MS, PANEL_HEIGHT_MS,
...
use super::panel_geometry::{
    ...
    resolve_native_panel_host_frame, resolve_native_panel_layout,
};
...
use crate::native_panel_core::{
    PanelHitAction, PanelHitTarget, PanelInteractionCommand, PanelPoint,
};
...
fn assert_macos_settings_display_row_click_dispatches_cycle_display_command(cards_visible: bool) {
    ...
    let display_region = regions
        ...
        .expect("island display cycle row region");
    let pointer = PanelPoint {
        x: display_region.frame.x + display_region.frame.width / 2.0,
        y: display_region.frame.y + display_region.frame.height / 2.0,
    };
    let mut state = panel_state();
    state.expanded = true;
    state.surface_mode = NativeExpandedSurface::Settings;
