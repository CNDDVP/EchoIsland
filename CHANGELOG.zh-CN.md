...
Windows 完整测试、安装器/便携版构建和真实混合 DPI 几何验证已通过。当前产物未进行代码签名，且 CNDDVP 专用 updater 签名材料未确立，因此更新保持 CN 发布页手动下载。原生文字像素截图、干净用户安装/升级/卸载生命周期、macOS 编译以外的运行验证，以及负坐标/竖屏/休眠恢复/Explorer 重启等扩展硬件矩阵仍待专用环境验证。旧管理员级 MSI 需先卸载再迁移当前用户安装。
