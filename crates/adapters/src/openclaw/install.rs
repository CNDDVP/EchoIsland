...
const HTTP_TRANSPORT: &str =
    include_str!("../../../../integrations/openclaw-plugin/echoisland-http.mjs");
...
pub fn install_openclaw_adapter(paths: &OpenClawPaths) -> Result<OpenClawStatus> {
    ...
    for directory in [&paths.hook_dir, &paths.plugin_dir] {
        let transport_path = directory.join("echoisland-http.mjs");
        fs::write(&transport_path, HTTP_TRANSPORT)
            .with_context(|| format!("failed to write {}", transport_path.display()))?;
        let locales = directory.join("locales");
        fs::create_dir_all(&locales)?;
        fs::write(
            locales.join("zh-CN.json"),
            include_str!("../../../i18n/locales/zh-CN.json"),
        )?;
        fs::write(
            locales.join("en-US.json"),
            include_str!("../../../i18n/locales/en-US.json"),
        )?;
    }
...
pub fn get_openclaw_status(paths: &OpenClawPaths) -> Result<OpenClawStatus> {
    let hook_installed = paths.hook_manifest_path.exists()
        && paths.hook_dir.join("echoisland-http.mjs").is_file()
        && hook_has_echoisland_marker(&paths.hook_handler_path)?;
    ...
    let plugin_installed = paths.plugin_manifest_path.exists()
        ...
        && paths.plugin_dir.join("echoisland-http.mjs").is_file()
    ...
    let support = supported_with_note(echoisland_i18n::t("adapter.openclaw_status"));
...
fn plugin_enabled(paths: &OpenClawPaths) -> Result<bool> {
    ...
    let allowed = root
        ...
        .is_none_or(|allow| {
            allow.as_array().is_some_and(|items| {
                items
                    .iter()
                    .any(|id| id.as_str() == Some(OPENCLAW_PLUGIN_ID))
            })
        });

    Ok(plugins_enabled && load_path_present && entry_enabled && !denied && allowed)
}
...
const postEvent = createTransport({{
  ...
  statusPath: RECEIVER_STATUS_PATH,
...
export default async function handler(input) {{
  ...
  try {{
    ...
    if (isBlockingEvent(envelope.hook_event_name)) return response;
  }} catch (_error) {{
  }}
}}
"#
...
fn render_plugin_manifest() -> String {
    json!({
        "id": "echoisland",
        "name": "EchoIsland",
        "description": echoisland_i18n::t("integration.openclaw_description"),
