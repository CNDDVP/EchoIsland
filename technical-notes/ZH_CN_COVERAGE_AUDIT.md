...
审计日期：2026-09-05。范围：EchoIsland v0.7.0 升级工作区；结论综合源码、词库、自动测试、Windows 构建产物和真实混合 DPI 运行数据。该版本尚未发布；原生文字像素截图、干净环境安装生命周期与 macOS 仍属明确的未验证范围。
...
- 本报告仍不宣称已通过：中文/英文/数字/Emoji 实际像素截图检查、负坐标多屏、竖屏、其他缩放率文字无遮挡、睡眠恢复或 Explorer 重启、全新用户安装/升级/卸载。这些需要相应硬件、可访问的原生 UI surface 或干净虚拟机。
...
| 原生 UIA 标签与硬件视觉矩阵 | 不属于残留英文翻译；是尚未验证或待增强的体验项 | 继续跟踪，不虚报完成 |
