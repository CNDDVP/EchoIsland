...
## v0.6.3 更新内容（2026-09-05）

1. **加固：悬浮条定位完全由目标显示器的物理边界推导**
   - 在 v0.6.2 的 DPI 修正 + 边界钳制基础上，面板 x/y 改为直接取目标显示器物理矩形做顶部居中（`x = 显示器.x + (显示器宽 - 面板宽)/2`，`y = 显示器.y`），彻底不依赖上游逻辑坐标换算的数值。混合 DPI 多屏、负坐标副屏、超大面板等场景均由单元测试覆盖。
   - 实测：4K 150% 主屏 + 1080p 100% 副屏，选择副屏后面板精准落在副屏顶部正中央。
